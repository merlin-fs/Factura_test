...
namespace FTg.Common.Editor
{
    abstract class BaseReferenceDrawer : PropertyDrawer
    {
        ...
        private void DrawCaret(Rect pickerRect)
        {
            ...
            {
                var packageInfo = UnityEditor.PackageManager.PackageInfo.FindForAssembly(typeof(BaseReferenceDrawer).Assembly);
                if (packageInfo != null)
                {
                    var iconPath = $"{packageInfo.assetPath}/Editor/Icon/PickerDropArrow-Pro.png";
                    m_CaretTexture = AssetDatabase.LoadAssetAtPath<Texture2D>(iconPath);
                }
            }
