...
namespace Game.Core.Services
{
    ...
    {
        // Exposed to inner states via context reference (ctx = this).
        ...
        public GameFlowService(float levelLength)
        {
        ...
        [Inject]
        private void Inject(GameSession session, SessionCoordinator coordinator)
        {
            _session     = session;
        ...
        private sealed class PlayingState : IState<GameFlowService, GameState>
        {
            ...
            public void Enter(GameFlowService ctx)
            {
                ctx._session.IsPaused = false;
                ctx._session.SetState(GameState.Playing);
            }
        ...
        private sealed class WinState : IState<GameFlowService, GameState>
        {
            ...
            public void Enter(GameFlowService ctx)
            {
                ctx._session.IsPaused = true;
                ctx._session.SetState(GameState.Win);
            }
        ...
        private sealed class LoseState : IState<GameFlowService, GameState>
        {
            ...
            {
                ctx._session.IsPaused = true;
                ctx._session.SetState(GameState.Lose);
            }
